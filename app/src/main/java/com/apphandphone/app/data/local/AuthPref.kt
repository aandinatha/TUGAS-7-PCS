package com.apphandphone.app.data.local

import android.content.Context
import android.content.SharedPreferences
import com.apphandphone.app.data.model.ActionState
import com.apphandphone.app.data.model.AuthUser
import com.apphandphone.app.util.getObject
import com.apphandphone.app.util.putObject
...
class AuthPref(val context:Context) {
    ...
    suspend fun login(email: String, password: String): ActionState<AuthUser>{
        ...
        return if (user == null){
            ...
            ActionState(message = "Email dan Password Tidak Boleh Koseng", isSuccess = false)
        }else if (user.email == email && user.password == password){
            isLogin = true
            ActionState(message = "Anda Berhasi Login")
        }else{
